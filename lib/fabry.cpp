...
: com_{&com}, size_{size} {
	...
	CMPI_Win_allocate(size * unit, unit, info,
			lift<MPI_Comm>(com.get_handle()), &mapping_, &cWin);
	// p_ = decay(win);
}
...
void active_rdma_base::pre_fence(no_root_tag) {
	MPI_Win_fence(MPI_MODE_NOPRECEDE, cWin.win);
}

void active_rdma_base::fence(no_root_tag) {
	MPI_Win_fence(0, cWin.win);
}

void active_rdma_base::post_fence(no_root_tag) {
	MPI_Win_fence(MPI_MODE_NOSUCCEED, cWin.win);
}
...
: r_{&r} {
	MPI_Win_lock(MPI_LOCK_EXCLUSIVE, r_->com_->rank(), MPI_MODE_NOCHECK, r_->cWin.win);
}

passive_rdma_base_scope::~passive_rdma_base_scope() {
	MPI_Win_unlock(r_->com_->rank(), r_->cWin.win);
}
